...
const pkceStore = new Map<string, { verifier: string; returnTo: string }>();
...
export async function GET(req: NextRequest) {
  ...
  if (action === 'start') {
    ...
    pkceStore.set(state, { verifier, returnTo });
  ...
  if (action === 'callback') {
    ...
    if (!saved) return new NextResponse('Invalid or expired state', { status: 400 });
    ...
    sessionStore.set(sessionId, {
      ...
      subdomain: tok.subdomain,
      obtained_at: Date.now(),
      controlPlane: CONTROL_PLANE,
    });
    pkceStore.delete(state);

    const back = new URL(saved.returnTo);
